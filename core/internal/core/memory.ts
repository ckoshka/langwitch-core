...
export const fit = (c: Memory) =>
	(recordedAt: Hours, accuracy: number) =>
...
export const remodel = (flexibility = 0.3) =>
	(estimatedCurve: number, oldDecay: number) =>
		(estimatedCurve * flexibility) + oldDecay * (1 - flexibility);

export const predict = (when: Hours) =>
	(c: Memory) =>
		use<MemoryConstantsReader>()
			.map2((f) =>
